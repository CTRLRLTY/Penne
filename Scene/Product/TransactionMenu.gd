...
func _show_page(products : Array) -> void:	
	...
	for product in products:
		var product_item : Control = ProductCardScene.instance()
		product_item.connect("amount_changed", self, "_on_Item_amount_changed")
		product_grid.add_child(product_item)
		product_item.change_attr(product, amount_cache[ctr])
		ctr += 1
